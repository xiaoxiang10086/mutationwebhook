...
func (handler *Handler) HandleAdmission(writer http.ResponseWriter, request *http.Request) {
	...
	if patchOperations, err := handler.Process(req); err != nil {
...
func (handler *Handler) Process(request *admissionv1.AdmissionRequest) ([]PatchOperation, error) {
	switch request.Operation {
	case admissionv1.Create:
		return handler.Handler.handleAdmissionCreate(request)
	case admissionv1.Update:
		return handler.Handler.handleAdmissionUpdate(request)
	case admissionv1.Delete:
		return handler.Handler.handleAdmissionDelete(request)
	default:
